%% h = sepia_handle_panel_swismwi_dataIO(hParent,h,position)
%
% Input
% --------------
...
%         
    ...
    case 'inputdata1'
        % only read NIfTI file for mask
        [fileName,pathDir] = uigetfile({'*.nii;*.nii.gz','NIfTI file (*.nii,*.nii.gz)'; '*.nii','NIfTI file (*.nii)'; '*.nii.gz','Gzipped NIfTI file (*.nii.gz)'},'Select a NIfTI file');
