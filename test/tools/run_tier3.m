...
%% run_tier3()
%
% Description: runs the Tier-3 toolbox-dependent regression matrix
% (test/tier3_matrix/) against the real dataset configured via
...
suite  = TestSuite.fromFolder(fullfile(testRoot, 'tier3_matrix'), 'IncludingSubfolders', true);
runner = TestRunner.withTextOutput();

tapFile = fullfile(testRoot, 'tools', 'tier3-results.tap');
...
assert(~any([results.Failed]), ...
    'run_tier3:testsFailed', 'One or more Tier-3 regression tests failed - see output above.');
