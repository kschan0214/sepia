%% TestBFRMatrix - Tier 3 real-dataset background-field-removal method regression matrix
%
...
classdef TestBFRMatrix < matlab.unittest.TestCase
    ...
    methods (Test)
        function testBFRMethodMatchesReference(testCase, bfrMethod)
            ...
            refFile = fullfile(sepiatest.test_root(), 'references', 'tier3', sprintf('bfr_%s.mat', slug));
            testCase.assumeTrue(isfile(refFile), ...
                sprintf(['No Tier-3 reference saved yet for BFR method "%s". ', ...
                         'Run regenerate_reference(''confirm'',true,''tier'',''tier3'') to create it after a manual review.'], bfrMethod));
