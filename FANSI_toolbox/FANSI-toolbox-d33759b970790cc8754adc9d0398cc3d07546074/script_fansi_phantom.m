...
imagesc3d2(phase_use, N/2, 2, [90,90,90], [-0.4 0.4], 0, ['Noise RMSE: ', num2str(rmse_noise)]) %-0.04,0.04
...
imagesc3d2(real(outf.x) .* mask_use - (mask_use==0), N/2, 11, [90,90,90], [-0.12,0.12], 0, ['FANSI-TV RMSE: ', num2str(rmse_f1), '  iter : ', num2str(outf.iter)])
...
imagesc3d2(real(outf2.x) .* mask_use - (mask_use==0), N/2, 12, [90,90,90], [-0.12,0.12], 0, ['FANSI-TGV RMSE: ', num2str(rmse_f2), '  iter : ', num2str(outf2.iter)])
