...
% G = G/sum(G(:));
% phi_h = real(ifftn(fftn(G).*fftn(params.input)));
...
for t = 1:num_iter
    ...
    x = mask.*(zm-sm);
    %mask.*real(ifftn( (mu * (tx + ty + tz) + Dt_kspace) ./ (eps + mu2*K2 + mu * EE2) ));
    ...
    if t < num_iter
        ...
        zm = real(ifftn( (mu * (tx + ty + tz) + Dt_kspace +mum*fftn(mask.*x+sm) ) ./ (eps + mu2*K2 + mu * EE2+mum) ));
