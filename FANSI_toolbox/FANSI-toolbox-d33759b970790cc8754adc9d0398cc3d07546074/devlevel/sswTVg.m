...
for t = 1:num_iter
    ...
    if t < num_iter
        ...
        %Fphi_h = (muh * conj(Lap).*fftn(z_h-s_h) + mu2*fftn(z2-s2) - mu2*kernel.*Fx +muh2*(conj(G).*fftn(z_h2 - s_h2)-fftn(z_h2 - s_h2)) ) ./ (eps + mu2 + muh * Lap.*conj(Lap) + muh2*(conj(G).*G-conj(G)-G+1) ) ;
        Fphi_h = (mu2*fftn(z2-s2) - mu2*kernel.*Fx +muh2*(conj(G).*fftn(z_h2 - s_h2)-fftn(z_h2 - s_h2)) ) ./ (eps + mu2 + muh2*(conj(G).*G-conj(G)-G+1) ) ;
        phi_h = real(ifftn(Fphi_h));
        
        %z_h = muh*(real(ifftn(Lap.*Fphi_h))+s_h)./(muh+beta*mask);
        z_h2 = muh2*(real(ifftn(G.*Fphi_h)-phi_h)+s_h2)./(muh2+gamma*mask);
        
        s2 = s2 + real(ifftn(kernel.*Fx)) - z2+phi_h;
        %s_h = s_h + real(ifftn(Lap.*Fphi_h)) - z_h;
        s_h2 = s_h2 + real(ifftn(G.*Fphi_h)) - phi_h - z_h;
    end
end
out.time = toc;toc
