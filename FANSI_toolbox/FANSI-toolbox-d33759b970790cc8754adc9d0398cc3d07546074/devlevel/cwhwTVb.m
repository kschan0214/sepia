...
G = G/sum(G(:));
gy = real(ifftn(fftn(G).*fftn(params.input.*mask)));
phi_h = gy;
...
for t = 1:num_iter
    ...
    if t < num_iter
        ...
        phi_h = ( mu2*(z2-s2)-mu2*real(ifftn((kernel.*fftn(mask.*z))))+beta*mask.*gy+gamma*real(ifftn((kernel.*fftn((1-mask).*z)))) )./(mu2+beta*mask+gamma);
...
%     
%             if z_update < 2*tol_update%(min(2*x_update,10*tol_update))
%                break
%             end
