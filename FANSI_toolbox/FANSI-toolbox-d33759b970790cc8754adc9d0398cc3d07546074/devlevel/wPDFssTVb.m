...
G = fftn(G);
    ...
    %params.input.*weight;
    if precond
        z_e = params.input.*weight;%real(ifftn(G.*fftn(params.input.*params.weight)));
        z_l = (params.input-z_e).*weight./(weight+mu_l);
    else
        z_e = zeros(N,'single');
        z_l = zeros(N,'single');
    end
...
for t = 1:num_iter
    ...
    if t < num_iter
        ...
        rhs2 = Wy*(1+beta) + mu_e*(s_e+real(ifftn(kernel.*fftn(y_e))));
        
        z_e = (rhs2-weight.*z_l)./ a3;
