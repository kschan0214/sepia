function levelUpdated = thresh_tool(image,cmap)
...
max_colors=max(image(:));    %practical limit
im=round(image/max_colors*512);
...
h_fig=figure('Name','User Guided Thresholding','NumberTitle','off');
...
subimage(uint8(im),double(full_map))
...
title('Adjust Intensities')

%very bottom - colorbar
h_cbar=axes('unit','norm','pos',[0.05 0.05 0.9 0.05],'tag','thresh_tool_cbar');
subimage(uint8(color_range),double(full_map))
% subimage(uint8(color_range),[0.5 1.5],1:num_colors,double(full_map))
...
levelUpdated=level/512*max_colors;
