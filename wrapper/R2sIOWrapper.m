%% chi = R2sIOWrapper(input,output,maskFullName,algorParam)
%
% Input
% --------------
...
%
%
function [r2s,t2s,s0] = R2sIOWrapper(input,output,maskFullName,algorParam)
%% add general Path
sepia_addpath
...
%% Dipole inversion
magnitude   = double(load_nii_img_only(availableFileList.magnitude));
if ~isempty(availableFileList.mask)
...
if isstruct(input)
    ...
    filePattern = {'mag','mag','','header'}; % don't change the order
    ...
    if ~isLoadSuccessful
        ...
        inputNiftiList = read_bids_to_filelist_magn_only(inputDir,fullfile(outputDir,prefix));
    end
...
if ~isempty(inputFileList(2).name)
    ...
    error('Fail! \nPlease specify a multi-echo(4D) magnitude data.');
end
