%% get_CIT168_subcortical_labels_in_native_space(input,mask,output_dir,algorParam)
%
% Usage:
%
% Input
...
%
%
function get_CIT168_subcortical_labels_in_native_space(input,output_dir,algorParam)
...
shell_script = fullfile(SEPIA_ANALYSIS_SEGMENTATION_dir,'ANTs_get_CIT168_labels.sh');
...
switch mode
    case 1      % registration is required
        ...
        cmd = ['sh ' shell_script ' ' num2str(mode) ' ' output_dir ' ' GRE_nii ' ' GRE_mask_nii ' ' T1w_nii ' ' T1w_mask_nii ' ' num2str(isBiasFieldCorr)];
    ...
    case 2      % transformation is provided
        ...
        cmd = ['sh ' shell_script ' ' num2str(mode) ' ' output_dir ' ' gre_2_t1w_mat ' ' t1_2_mni2009c_mat ' ' t1_2_mni2009c_inverseWrap_nii ' ' T1w_nii];
end
