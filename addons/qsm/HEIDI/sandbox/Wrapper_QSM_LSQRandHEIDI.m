...
HEIDI_HOME = fullfile(SEPIA_HOME,'..','external','HEIDI');                                      % temporary hard-coded, will update
...
% sepia_addpath('LSQR');
% addpath(fullfile(SEPIA_HOME,'misc','qsm_algorithm','LSQR'));

% 20250614 KC: magn need to be 3D, compress multu-echo data as MEDI
magn = sqrt(sum(abs(magn).^2,4)); magn = magn ./ mean(magn(mask>0));
...
% 20250614: convert local field unit for HEIDI
phase2susceptibilityFactor=phase2susceptibilityfactor([], options);
localField = localField /(headerAndExtraData.sepia_header.B0*gyro) /phase2susceptibilityFactor/1e6;

% main - LSQR
[chi_raw,~, ~,~,~,~,~,~,~,phaseDiscrepancy] = computesusceptibility_lsqr(localField,options,logical(mask),[],spatialWeightingMatrix);

%% Now HEIDI begins
% 20250614: don't need to add path for addon
rmpath(fullfile(HEIDI_HOME,'LSQR'));
% rmpath('LSQR');
% rmpath(fullfile(SEPIA_HOME,'misc','qsm_algorithm','LSQR'));
% 
% sepia_addpath('HEIDI');
% addpath(fullfile(SEPIA_HOME,'misc','qsm_algorithm','HEIDI'));
...
options.PostProcCone.threshold                  = algorParam.qsm.PostProcCone.threshold;
options.PostProcCone.tol                        = algorParam.qsm.PostProcCone.tol;
options.PostProcCone.tolEnergy                  = algorParam.qsm.PostProcCone.tolEnergy; % makes much better results

% main - HEIDI
[chi,~, ~,~,~,~,~,~,~,phaseDiscrepancy] = computesusceptibility_heidi(chi_raw,localField,options,logical(mask),[],spatialWeightingMatrix);
...
% 20250614: making the chi contrast consisent as the rest
chi = -chi;
...
function algorParam2 = check_and_set_algorithm_default(algorParam,options)
    ...
    try algorParam2.qsm.PostProcCone.threshold = algorParam.qsm.PostProcCone.threshold; % HEIDI
        catch; algorParam2.qsm.PostProcCone.threshold = 0.1; end
    try algorParam2.qsm.PostProcCone.tol = algorParam2.qsm.PostProcCone.tol; % HEIDI
        catch; algorParam2.qsm.PostProcCone.tol = eps; end
    try algorParam2.qsm.PostProcCone.tolEnergy = algorParam2.qsm.PostProcCone.tolEnergy; % HEIDI
        catch; algorParam2.qsm.PostProcCone.tolEnergy = eps; end
