...
%% it is for negtive w   2014.1.13
% add voxel_size    2014/06/13
...
function [kappa,m_fat,unwph,iter,erglist,kappa_iter] = unwrap_unfat_3dP(voxel_size,iMag,w,psi,p,varargin)
...
% kappa = zeros(m,n,h); % Initial labeling  when w is negtive, initial labeling is w.

% whos('w')
% test_1 = ones(m,n,h);
% whos('test_1')

kappa = -w*ones(m,n,h); % Initial labeling  when w is negtive, initial labeling is w.
kappa_fat = mod(kappa,1)~=0;
% for i = 1:h
...
%     erg_previous = energy_ho_3d_pi(kappa,psi,base_xy,base_z,p,cliques,disc_bar,disc_bar_z,th,quant);
    erg_previous = energy_ho_3dW2_pi(voxel_size,iMag,kappa,psi,base_xy,base_z,p,cliques,disc_bar,disc_bar_z,th,quant);
...
% the negtive w, the jump length should be different
        base_kappa_jump_xy = (2-w)*(1 - base_kappa_fat_xy) + w*(base_kappa_fat_xy);
...
%         base_kappa_jump_z = (2-w)*base_kappa_fat_z + w*(1-base_kappa_fat_z);
        % for negtive w
        base_kappa_jump_z = (2-w)*(1 - base_kappa_fat_z) + w*(base_kappa_fat_z);
...
%             BB(:,:,zz) = clique_energy_ho(abs(-binaryEdge_term*jump_size + b(:,:,zz)),p,th,quant).*base_z(:,:,zz).*base_disc_bar_z(:,:,zz);
        ...
        kappa_aux = kappa + kappa_temp.*((w)*kappa_fat + (2-w)*(1-kappa_fat));
...
%       erg_actual = energy_ho_3d_pi(kappa_aux,psi,base_xy,base_z,p,cliques,disc_bar,disc_bar_z,th,quant);
        erg_actual = energy_ho_3dW2_pi(voxel_size,iMag,kappa_aux,psi,base_xy,base_z,p,cliques,disc_bar,disc_bar_z,th,quant);
        %
        if (erg_actual < erg_previous)
            erg_previous = erg_actual;
            kappa = kappa_aux;
%             kappa_iter = [kappa_iter kappa];
