% Single-Step QSM with a Total Generalized Variation regularization with spatially variable fidelity and regularization weights.
% This uses ADMM to solve the functional.
%
% Last modified by Carlos Milovic in 2017.06.09
% bug fix: line 123 KC (2 March 2020)
%
function out = WH_wTGV_4sepia(params)
%
...
%
    ...
    K = params.K;
    FWy = fftn(weight.*params.input./(weight+mu2));
    ...
    KtK_mu1_E_sos    = mu2*Kt.*K + mu1*(E1tE1 + E2tE2 + E3tE3);
    ...
    a1 = KtK_mu1_E_sos; a2 = mu1I_mu0_E_wsos1;  a3 = mu1I_mu0_E_wsos2;  a4 = mu1I_mu0_E_wsos3;
    ...
    for outer = 1:maxOuterIter
        ...
        rhs1    =  mu2*Kt.*(z2-s2-fftn(phi_h))     + mu1*(Et1.*F_z1_minus_s1_1 + Et2.*F_z1_minus_s1_2 + Et3.*F_z1_minus_s1_3);
        ...
        z2 = FWy + mu2*fftn( real(ifftn(K.*Fx+s2)+phi_h)./(weight + mu2) );
        
        Fphi_h = (muh * conj(Lap).*fftn(z_h-s_h) + mu2*(z2-s2) - mu2*K.*Fx) ./ (eps + mu2 + muh * Lap.*conj(Lap)) ;
